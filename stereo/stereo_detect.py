...
from stereo import stereoconfig  # 导入相机标定的参数
...
def getRectifyTransform(height, width, config):
    ...
    # R1、R2分别是摄像机的校正变换矩阵（旋转变换）；P1、P2分别是摄像机在新坐标系下的投影矩阵；Q是4*4的深度差异映射矩阵
    R1, R2, P1, P2, Q, roi1, roi2 = cv2.stereoRectify(left_K, left_distortion, right_K, right_distortion,
                                                      (width, height), R, T, alpha=0)

    # initUndistortRectifyMap()该函数功能是计算畸变矫正和立体校正的映射变换。
...
def stereoMatchSGBM(left_image, right_image, down_scale=False):
    ...
    if not down_scale:
        disparity_left = left_matcher.compute(left_image, right_image)  # compute函数计算视差图
...
if __name__ == '__main__':
    ...
    # 读取数据集的图片
    iml = cv2.imread("../left_img_40.png")  # 左图
    imr = cv2.imread("../right_img_40.png")  # 右图
    ...
    cv2.imwrite("../rec_img_40.png", line)

    # 1.消除畸变（注意下面原来的代码没有用立体校正后的图像）
    ...
    # iml = undistortion(iml_rectified, config.cam_matrix_left, config.distortion_l)
    # imr = undistortion(imr_rectified, config.cam_matrix_right, config.distortion_r)
    ...
    cv2.imwrite("../shicha_40.png", disp)
