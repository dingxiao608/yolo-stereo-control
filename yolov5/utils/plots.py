...
def plot_one_box(x, im, color=None, label=None, line_thickness=3,pos = 'top'):
    ...
    if label :
        if pos == 'top':
            ...
            c2 = c1[0] + t_size[0], c1[1] + t_size[1] + 3
            cv2.rectangle(im, c1, c2, color, -1, cv2.LINE_AA)  # filled
            cv2.putText(im, label, (c1[0], c2[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
...
def plot_study_txt(path='', x=None):  # from utils.plots import *; plot_study_txt()
    # Plot study.txt generated by stereo_detect.py
