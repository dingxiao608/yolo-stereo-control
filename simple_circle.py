...
# 速度控制 for 450 大概一圈，一圈32条数据，只有1/3的时间检测到数据
        ...
        if has_object:
            ...
            if finite and valid:
                ...
                if pos_NED[2] < 0 and np.linalg.norm(pos_NED, ord=2) < 50:
                    ...
                    # cv2.waitKey()
                    # 保存图片，生成数据集
                    strl = "image/simple_circle/left/" + str(value) + ".png"
                    strr = "image/simple_circle/right/" + str(value) + ".png"
...
Note = open('image/simple_circle/real_pos.txt', mode='w')
Note.write(str(real))
Note = open('image/simple_circle/det_pos.txt', mode='w')
Note.write(str(det))
Note.close()
